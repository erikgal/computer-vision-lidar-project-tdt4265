...
class BiFPN(nn.Module):
    def __init__(self,  fpn_sizes, output_channels):
        ...
        self.p6_upsample = nn.Upsample(scale_factor=2, mode='nearest').cuda()

        self.p5_td_conv  = nn.Conv2d(P5_channels, self.W_bifpn, kernel_size=3, stride=1, bias=True, padding=1).cuda().cuda()
...
class biFPN(torch.nn.Module):
        def __init__(self,
            ...
            self.layer5 = nn.Sequential(
                nn.ReLU().cuda(),
                nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, stride=1, padding=1).cuda(),
                nn.ReLU().cuda(),
                nn.Conv2d(in_channels=512, out_channels=512, kernel_size=3, stride=2, padding=1).cuda(),
                nn.ReLU().cuda()
            )

            self.layer6 = nn.Sequential(
                nn.ReLU().cuda(),
                nn.Conv2d(in_channels=512, out_channels=512, kernel_size=2, stride=1, padding=1).cuda(),
                nn.ReLU().cuda(),
            ...
            self.m = []
            self.m.append(BiFPN([64, 128, 256, 512, 512, 512], out_channels))
            for i in range(nbr_layers-1):
                self.m.append(BiFPN([out_channels, out_channels, out_channels, out_channels, out_channels, out_channels], out_channels))
